...
(function($) {
    ...
    $.popup = function(options) {
        ...
        var method = {
            ...
            mask: function() {
                ...
                if (mask.length > 0) {
                    ...
                    mask.css({zIndex: sets.zindex}).stop(true, true).fadeIn(200);
                } else {
            ...
            setCss: function(param) {
                ...
                if (sets.width) sets.popup.css('width', sets.width + 'px');
                if (sets.height) sets.popup.css('height', sets.height + 'px');
                if (sets.zindex) {
                    sets.popup.css({zIndex: Number(sets.zindex) + 10});
                } else {
                    zindex = sets.mask.css('z-index');
                    sets.popup.css({zIndex: Number(zindex) + 10});
                }
            },
            // 倒计时关闭
            ...
            close: function(obj, noscroll) {
                ...
                // 跳转
                if (sets.jump) document.location = sets.jump;
                // 设置弹层属性
                ...
                // 关闭回调
                if (sets.callback) sets.callback();
            },
            esc: function(popup) {
        ...
        if (popup.length > 0) {
            ...
            mask = $('#' + sets.mask);
            // 显示自有弹层并添加属性
            popup.hide().stop(true, true).fadeIn(200, function() {
                popup.attr({'popup-status': 'show', 'popup-mark': 'own'});
            });
            // 禁止页面滚动
